...
mydb = mysql.connector.connect(host="localhost",user="root",password="123456",database="KL")	
...
def getdata():
	if request.method=="POST":
		...
		if data9:
			...
			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=1 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
			data1=cursor1.fetchall()
			cursor1.close()
			
			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=2 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
			data2=cursor1.fetchall()
			cursor1.close()

			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=3 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
			data3=cursor1.fetchall()
			cursor1.close()

			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=4 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
			data4=cursor1.fetchall()
			cursor1.close()

			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=5 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
			data5=cursor1.fetchall()
			cursor1.close()

			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=6 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
			data6=cursor1.fetchall()
			cursor1.close()

			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=7 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
			data7=cursor1.fetchall()
			cursor1.close()
	
			cursor1=mydb.cursor(buffered=True)
			cursor1.execute("select modelname,operation,heatno,produced,passed,reject,lossno,lossinmin,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejctioncount,hasloss from hourlydata where hour=8 and empcode=%s and date=%s and shift=%s",(empcode,date,shift,)) 
...
def datastore():
	if request.method=="POST":
		...
		print(totalmaterialrejectioncount)
		print(totalprocessrejectioncount)
		...
		cursor3=mydb.cursor()	
		cursor3.execute("insert into hourlydata values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(empcode,empname,machinecode,modelname,operation,date,shift,hour,heatno,produce,passed,reject,materialrejectiontype,materialrejectioncount,processrejectiontype,processrejectioncount,hasloss,lossno,lossinmin,))
		mydb.commit()
		...
		cursor3.execute("INSERT INTO dailydata(empcode,empname,machinecode,supervisor,modelname,operation,date,shift,produced,passed, reject,materialrejectioncount,processrejectioncount,lossinmin) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE produced=%s,passed=%s,reject=%s,materialrejectioncount=%s,processrejectioncount=%s,lossinmin=%s",[empcode,empname,machinecode,supervisor[0],modelname,operation,date,shift,totalproduce,totalpassed,totalreject,totalmaterialrejectioncount,totalprocessrejectioncount,totallossinmin,totalproduce,totalpassed,totalreject,totalmaterialrejectioncount,totalprocessrejectioncount,totallossinmin])
		mydb.commit()
...
def assignmachine():
	...
	cursor1.close()
	return render_template("assignmachines.html",operator=operator,machine=machine)
...
def showassignedmachines():
	if request.method=="POST":
		machinecode=request.form['machinecode']
		cursor1=mydb.cursor(buffered=True)	
		cursor1.execute("SELECT machinecode,empcode,shift  FROM assignedduties where machinecode=%s",[machinecode])
		data=cursor1.fetchall()
		cursor1.close()
		return jsonify({'data':data})
...
@login_required
def viewduty():
	return render_template('viewduty.html')





@app.route('/showduty',methods=["POST"])
@login_required
def showduty():
	if request.method=="POST":
		date=request.form['date']
		cursor1=mydb.cursor(buffered=True)	
		cursor1.execute("SELECT * from duties where date=%s",(date,))
		data=cursor1.fetchall()
		cursor1.close()
		return jsonify({'data':data})
